    <header>
                                ...
                                <nav id="mobile-menu" class="header__menu main-menu">
                                    <ul>
                                        ...
                                        <li class="has-dropdown">
                                            ...
                                            <ul class="sub-menu">
                                                <li><a href="{{url('user.profile', session('user')['id'])}}">Profile</a></li>
        ...
        @auth
            ...
            <div class="tpsideinfo__account-link">
                <a href="{{url('product/cart')}}"><i class="icon-cart icons"></i> My Cart</a>
            </div>
            <div class="tpsideinfo__account-link">
        ...
        @else
        <div class="tpsideinfo__account-link">
            <a href="{{url('auth.login')}}"><i class="icon-user icons"></i> Loginr</a>
        </div>
        @endauth
    ...
    $(document).ready(function(){
        function loadCart() {
            $.ajax({
                url: "{{ url('product/cart/load') }}",
                type: "GET",
                success: function(response) {
                    $('#cart-products').html(response.cart_details);
                    $('.tpcart__total-price .heilight-price').text(response.cart_subtotal);
                    $('.tpcart__checkout .tpcart-btn').attr('href', "{{url('product/cart')}}");
                    $('.tpcart__checkout .tpcheck-btn').attr('href', "{{ url('checkout') }}");
                    $('.tpcart__checkout #checkout-form').attr('action', "{{ url('checkout') }}");
                }
            });
        }
        ...
        $(document).on('click', '.tpcart__del a', function(e){
            ...
            var product_id = $(this).data('product-id');
            $.ajax({
                url: "{{ url('product/remove-cart') }}?product_id="+product_id,
                type: "GET",
                success: function(response) {
                    loadCart();
                }
